...
npm run index:search        # Build configured content indexes
npm run optimize:media      # Optimize supported local media files
npm run migrate:video-hls   # Prepare configured media for HLS playback
npm test                    # Run the test suite
```

Source ingestion, cross-host synchronization, backup, and one-off destructive
media migration tools are kept outside the public repository. Provide them
through a local, permission-restricted operations directory only when needed.

## Deployment Notes

- Keep `.env`, `data/`, `library/`, `backups/`, and uploaded assets outside Git.
