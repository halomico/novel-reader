...
test("filters realtime visits before counting and pagination", (t) => {
  ...
  insertEvent.run("novel_view", "/books/1", "203.0.113.1", "CN", novelId, null);
  insertEvent.run("video_view", "/media/1", "203.0.113.2", "US", null, videoId);
  insertEvent.run("audio_view", "/media/2", "203.0.113.3", "CN", null, audioId);
  insertEvent.run("video_view", "/media/1", "203.0.113.4", "unknown", null, videoId);
