...
export function entitlementTargetExists(definition: EntitlementDefinition): boolean {
  ...
  return Boolean(db.prepare("SELECT 1 FROM media_assets WHERE kind = ? AND folder = ? LIMIT 1").get(kind, folder));
}
...
export function listEntitlementTargets(
  ...
  if (targetType === "audio_folder" || targetType === "file_folder") {
    ...
    return db.prepare(
      `SELECT CASE WHEN folder = '' THEN '/' ELSE folder END AS id,
              CASE WHEN folder = '' THEN '根目录' ELSE folder END AS label,
              CAST(COUNT(*) AS TEXT) || ' 项' AS meta
       FROM media_assets
       WHERE kind = ? AND (? = '%%' OR folder LIKE ?)
       GROUP BY folder ORDER BY natural_sort_key(folder), folder LIMIT ?`,
  ...
  return db.prepare(
    `SELECT CAST(id AS TEXT) AS id, title AS label,
            CASE WHEN folder = '' THEN file_name ELSE folder || ' / ' || file_name END AS meta
     FROM media_assets
  ...
  if (targetType === "audio_folder" || targetType === "file_folder") {
    ...
    return (db.prepare(
      `SELECT CASE WHEN folder = '' THEN '/' ELSE folder END AS id,
              CASE WHEN folder = '' THEN '根目录' ELSE folder END AS label,
              CAST(COUNT(*) AS TEXT) || ' 项' AS meta
       FROM media_assets WHERE kind = ? AND folder = ? GROUP BY folder`,
