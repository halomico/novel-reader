...
test("navigation keeps the current surface while loading and paged readers avoid route flashes", () => {
  ...
  assert.match(read("src/app/styles/core.css"), /--font-brand: var\(--font-brand-face, "Inter Display"\)/);
  assert.match(read("src/app/layout.tsx"), /localFont\(\{\s*src: "\.\/fonts\/InterDisplay-Medium\.woff2"/);
