...
test("reserves peak space from the possible H.264 output instead of source size alone", () => {
  const sourceSize = 500 * 1024 * 1024;
  ...
  assert.ok(estimated > sourceSize * 2);
  assert.equal(
    estimatePlaybackHlsTemporaryBytes(sourceSize, {
      width: 1920,
      height: 1080,
      videoCodec: "h264",
      pixelFormat: "yuv420p",
