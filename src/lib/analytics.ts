...
type AnalyticsRealtimeOverview = Pick<
  AnalyticsOverview,
  | "activeNow"
  | "realtime"
  | "realtimePage"
...
type AnalyticsRealtimeActivity = Omit<AnalyticsRealtimeOverview, "realtimeCountries">;

export type AnalyticsSummaryOverview = Omit<AnalyticsOverview, keyof AnalyticsRealtimeOverview>;
...
export function getAnalyticsSummaryOverview(
  rangeValue: string | undefined,
...
export function getAnalyticsRealtimeActivity(
  rangeValue: string | undefined,
  options: AnalyticsOverviewOptions = {},
): AnalyticsRealtimeActivity {
  const filter = resolveAnalyticsTimeFilter(rangeValue, options.customFrom, options.customTo);
  ...
  const realtimeWhereSql = `${realtimeTime.sql} AND ${realtimeContent.sql} AND ${realtimeRegion.sql}`;
  const realtimeWhereParams = [...realtimeTime.params, ...realtimeContent.params, ...realtimeRegion.params];
...
export function getAnalyticsRealtimeCountries(
  rangeValue: string | undefined,
  options: AnalyticsOverviewOptions = {},
): string[] {
  const filter = resolveAnalyticsTimeFilter(rangeValue, options.customFrom, options.customTo);
  const realtimeTime = contentTimeWhere(filter, "e.created_at", "e.novel_id", "e.media_id");
  const realtimeContent = realtimeContentCondition(
    normalizeAnalyticsRealtimeContentFilter(options.realtimeContentType),
  );
  const rows = getDb()
    .prepare(
      `SELECT CASE
        ...
        END AS country
       FROM analytics_events e
       WHERE ${realtimeTime.sql} AND ${realtimeContent.sql}
       GROUP BY CASE
          WHEN e.country IS NULL OR TRIM(e.country) = '' OR LOWER(TRIM(e.country)) = 'unknown' THEN 'unknown'
          ELSE UPPER(TRIM(e.country))
        END
    ...
    .all(...realtimeTime.params, ...realtimeContent.params) as Array<{ country: string }>;
  return rows.map((row) => row.country);
}

export function getAnalyticsRealtimeOverview(
  rangeValue: string | undefined,
  options: AnalyticsOverviewOptions = {},
): AnalyticsRealtimeOverview {
  return {
    ...getAnalyticsRealtimeActivity(rangeValue, options),
    realtimeCountries: getAnalyticsRealtimeCountries(rangeValue, options),
  };
}
...
export function getAnalyticsOverview(
  ...
  return {
    ...getAnalyticsSummaryOverview(rangeValue, options),
    ...getAnalyticsRealtimeOverview(rangeValue, options),
  };
}
