...
export function SiteEntryNotice({ enabled, title, markdown, version }: SiteEntryNoticeProps) {
  ...
  useEffect(() => {
    ...
    try {
      if (
        window.localStorage.getItem(DISMISSED_NOTICE_KEY) === noticeVersion ||
        window.sessionStorage.getItem(SHOWN_NOTICE_KEY) === noticeVersion
      ) {
        setOpen(false);
        return;
      }
    ...
    if (shownVersionRef.current === noticeVersion) {
      setOpen(false);
      return;
    }

    shownVersionRef.current = noticeVersion;
    try {
      window.sessionStorage.setItem(SHOWN_NOTICE_KEY, noticeVersion);
    } catch {
      // The in-memory marker still prevents repeats while this layout remains mounted.
    }
