...
import { getPostgresSearchQueryDetails, normalizePostgresSearchAnalyticsQuery, type SearchQuerySource } from "@/domains/analytics/postgres-admin-analytics";
...
export default async function AdminSearchDetailPage({ searchParams }: SearchDetailPageProps) {
  ...
  const query = normalizePostgresSearchAnalyticsQuery(params.q || "");
  const details = await getPostgresSearchQueryDetails(database(), query, params.range, {
    page: params.page,
    pageSize: 30,
