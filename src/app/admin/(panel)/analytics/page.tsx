...
import { getPostgresAdminAnalyticsOverview, getPostgresReadingAnalytics, type AnalyticsMetric, type ReadingAnalytics } from "@/domains/analytics/postgres-admin-analytics";
...
export default async function AdminAnalyticsPage({ searchParams }: AdminAnalyticsPageProps) {
  ...
  const overviewPromise = getPostgresAdminAnalyticsOverview(database(), params.range, {
  ...
  const [overview, readingAnalytics] = await Promise.all([
    overviewPromise,
    getPostgresReadingAnalytics(database(), 30, 10),
  ]);
