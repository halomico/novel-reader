...
import { listPostgresMarketProducts, listPostgresRedemptionCodeBatches } from "@/domains/market/postgres-market";
import { AdminFrame } from "../../AdminFrame";

export const dynamic = "force-dynamic";
export const metadata: Metadata = { robots: { index: false, follow: false } };

export default async function AdminMarketCodesPage() {
  const [products, batches] = await Promise.all([
    listPostgresMarketProducts(database(), { includeUnpublished: true }),
    listPostgresRedemptionCodeBatches(database(), 100),
  ]);
  return (
