...
import { listPostgresUserLevelDefinitions, POSTGRES_USER_PERMISSION_DEFINITIONS } from "@/domains/identity/postgres-permissions";
import { saveUserLevelsAction } from "@/app/admin/actions";
...
export default async function AdminUserLevelsPage({ searchParams }: AdminUserLevelsPageProps) {
  ...
  const levels = await listPostgresUserLevelDefinitions(database("web"));
            ...
            {levels.map((level) => (
              <section className="adminUserLevelRow" key={level.level}>
                ...
                <div className="adminUserLevelPermissions">
                  {POSTGRES_USER_PERMISSION_DEFINITIONS.map((permission) => (
