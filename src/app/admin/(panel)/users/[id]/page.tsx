...
import { getPostgresUserById, listPostgresBrowseHistoryPage, listPostgresUserLoginRecordsPage } from "@/domains/identity/postgres-users";
import { clearAdminUserHistoryAction, deleteAdminUserHistoryAction } from "@/app/admin/actions";
...
export default async function AdminUserDetailPage({ params, searchParams }: AdminUserDetailPageProps) {
  ...
  const history = view === "history"
    ? await listPostgresBrowseHistoryPage(database("web"), user.id, { page: Number(query.historyPage || 1), pageSize: 20 })
    : null;
  const loginRecords = view === "login"
    ? await listPostgresUserLoginRecordsPage(database("web"), user.id, { page: Number(query.loginPage || 1), pageSize: 20 })
    : null;
  const entitlements = view === "rights"
    ? await listPostgresUserEntitlementsPage(database("web"), user.id, {
        page: Number(query.rightsPage || 1),
        pageSize: 20,
      })
    : null;
