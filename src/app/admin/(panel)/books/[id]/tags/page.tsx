...
import { saveNovelTaggingAction } from "@/app/admin/actions";
import { AdminFrame } from "../../../AdminFrame";
import { database } from "@/core/db/postgres";
import { getPostgresPublicNovel, listPostgresTagsForNovels } from "@/domains/catalog/postgres-catalog";
...
export default async function AdminBookTagPage({ params, searchParams }: AdminBookTagPageProps) {
  ...
  const book = await getPostgresPublicNovel(database(), bookId);
  if (!book) {
    notFound();
  }
  const [groups, tags, hotwords] = await Promise.all([
  ...
  const selectedTagIds = new Set((tags.get(book.id) ?? []).map((tag) => tag.id));
