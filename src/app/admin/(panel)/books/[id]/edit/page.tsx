...
import { getPostgresAdminNovel, getPostgresAdminNovelSource, isPostgresNovelInRecommendationPool, listPostgresAdminNovelChapters } from "@/domains/catalog/postgres-admin-novels";
import { getPostgresNovelRecommendationCount } from "@/domains/activity/postgres-recommendations";
import { database } from "@/core/db/postgres";
import { listPostgresCatalogTagGroups, type PostgresCatalogTagGroup as TagGroup, type PostgresCatalogTag as TagWithCount } from "@/domains/catalog/postgres-tags";
import { listPostgresTagsForNovels } from "@/domains/catalog/postgres-catalog";
import { listPostgresNovelHotwords } from "@/domains/reading/postgres-reader-catalog";
import { saveNovelEditorAction } from "@/app/admin/actions";
...
export default async function AdminBookEditPage({ params, searchParams }: AdminBookEditPageProps) {
  ...
  const book = await getPostgresAdminNovel(database(), bookId);
  if (!book) {
    notFound();
  }

  const [groups, selectedTags, hotwords, inRecommendationPool, source, chapterPreview] = await Promise.all([
    listPostgresCatalogTagGroups(database(), { audience: "admin" }),
    listPostgresTagsForNovels(database(), [book.id], { audience: "admin" }),
    listPostgresNovelHotwords(database(), book.id),
    isPostgresNovelInRecommendationPool(database(), book.id),
    book.source_id ? getPostgresAdminNovelSource(database(), book.source_id) : Promise.resolve(null),
    book.storage_mode === "chapters" ? listPostgresAdminNovelChapters(database(), book.id, 1, 20) : Promise.resolve(null),
  ]);
  const selectedTagIds = new Set((selectedTags.get(book.id) ?? []).map((tag) => tag.id));
  const recommendationCount = await getPostgresNovelRecommendationCount(database("web"), book.id);
                  ...
                  return (
                    <fieldset className="adminBookTagGroup" key={group.group?.id || "ungrouped"}>
                      ...
                      <div className="adminBookTagOptions">
                        {tags.map((tag) => (
                          <label className={tag.visibility !== "hidden" ? "adminBookTagOption" : "adminBookTagOption isMuted"} key={tag.id}>
