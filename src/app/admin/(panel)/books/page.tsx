...
import { listPostgresAdminBooks, listPostgresAdminNovelSources, listPostgresPinnedNovels } from "@/domains/catalog/postgres-admin-novels";
...
export default async function AdminBooksPage({ searchParams }: AdminBooksPageProps) {
  ...
  const uploadSources = await listPostgresAdminNovelSources(database(), true);
  ...
  const bookList = await listPostgresAdminBooks(database(), {
  ...
  const pinnedBooks = await listPostgresPinnedNovels(database());
