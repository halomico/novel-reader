...
import { listPostgresAdminNovelsByIds } from "@/domains/catalog/postgres-admin-novels";
...
import { batchUpdateNovelsAction } from "@/app/admin/actions";
...
export default async function AdminBookBatchPage({ searchParams }: AdminBookBatchPageProps) {
  ...
  const books = await listPostgresAdminNovelsByIds(database(), ids);
  if (!books.length) {
    notFound();
  }
  const groups = await listPostgresCatalogTagGroups(database(), { audience: "admin" });
