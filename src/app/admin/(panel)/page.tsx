...
} from "@/domains/identity/postgres-admin-overview";
...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  const [params, headerStore, settings] = await Promise.all([
    searchParams,
    headers(),
    readPostgresSiteSettings(),
  ]);
  const access = getAdminAccessState(headerStore);
  const [stats, { summary: indexStats }, loginRecords] = await Promise.all([
    getPostgresAdminCatalogStats(database("web")),
    readPostgresContentIndexStatus(database("web"), settings.novelSourceSearchModes),
    listPostgresAdminLoginRecordPage(database("web"), Number(params.loginPage || "1")),
  ]);
      ...
      <section className="adminHome">
        ...
        <div className="adminHomeGrid">
          ...
          <Link className="adminHomeLink" href="/admin/indexes">
            ...
            <span>
              <strong>搜索索引</strong>
              <small>构建和维护 PostgreSQL 中文全文索引</small>
            </span>
          </Link>
