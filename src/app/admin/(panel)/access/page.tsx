import { ChevronDown, Clock3, Gauge, Plus, Save, Trash2 } from "lucide-react";
...
export default async function AdminAccessPage({ searchParams }: AccessPageProps) {
  ...
  const [rules, policies] = await Promise.all([
    listPostgresContentAccessRules(database("web")),
    listPostgresContentAccessPolicies(database("web")),
  ]);
