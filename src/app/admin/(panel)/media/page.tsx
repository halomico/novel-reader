...
export default async function AdminMediaPage({ searchParams }: AdminMediaPageProps) {
  ...
  const categories = kind === "video" ? await listPostgresVideoCategories(database("web"), { includeHidden: true }) : [];
  const videoTags = kind === "video"
    ? (await listPostgresVideoTags(database("web"), { includeHidden: true, pageSize: 5_000 })).tags
    : [];
  ...
  const result = view === "assets"
    ? await listPostgresMediaAssets(database("web"), {
        kind,
  ...
  const kindFolders = view === "assets" ? await listPostgresMediaFolders(database("web"), kind) : [];
  const folders: Record<MediaKind, MediaFolder[]> = {
  ...
  const tagsByAsset = kind === "video"
    ? await listPostgresVideoTagsForAssets(database("web"), result.assets.map((asset) => asset.id))
    : {};
