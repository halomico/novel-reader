...
import {
  listPostgresMarketProducts,
  listPostgresUserMarketOrders,
...
export default async function MarketPage({ searchParams }: MarketPageProps) {
  ...
  const products = view === "products" ? await listPostgresMarketProducts(database()) : [];
  const orders = view === "orders" ? await listPostgresUserMarketOrders(database(), user.id, 100) : [];
