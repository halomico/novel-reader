...
import { getPostgresTelegramUserLink } from "@/domains/notifications/postgres-telegram-links";
...
import {
  getPostgresStationThread,
  listPostgresStationMessages,
  listPostgresUserStationThreads,
...
import { unlinkTelegramAction } from "@/app/messages/actions";
import { MessageReadTracker } from "@/components/MessageReadTracker";
...
export default async function MessagesPage({ searchParams }: MessagesPageProps) {
  ...
  const canMessageStation = await hasPostgresUserPermission(database("web"), user, "station_message");
  const tab = params.tab === "station" ? "station" : "announcements";
  const announcements = tab === "announcements" ? await listPostgresVisibleAnnouncements(database("web"), true) : [];
  const threads = tab === "station" ? await listPostgresUserStationThreads(database("web"), user.id) : [];
  const selectedThreadId = Number(params.thread || 0);
  const selectedThread = tab === "station" && selectedThreadId > 0
    ? await getPostgresStationThread(database("web"), selectedThreadId, { userId: user.id })
    : null;
  const messages = selectedThread ? await listPostgresStationMessages(database("web"), selectedThread.id) : [];
  const telegramAvailable = isTelegramUserLinkAvailable();
  const telegramLink = telegramAvailable ? await getPostgresTelegramUserLink(database("web"), user.id) : null;
  ...
  return (
    <>
      <MessageReadTracker threadId={selectedThread?.id} />
      {params.notice ? (
        <DismissibleNotice
          ...
          <section className={selectedThread ? "stationChatWorkspace hasSelection" : "stationChatWorkspace"}>
            <aside className="stationChatSidebar">
              ...
              <nav className="stationThreadList" aria-label={tr("站务对话")}>
                {threads.map((thread) => (
                  <Link className={`${selectedThread?.id === thread.id ? "isActive" : ""}${thread.unreadForUser ? " isUnread" : ""}`.trim()} href={`/messages?tab=station&thread=${thread.id}`} key={thread.id}>
                    <span>
