...
import { getPostgresUserGrowthProgress, POSTGRES_USER_PERMISSION_DEFINITIONS } from "@/domains/identity/postgres-permissions";
import Link from "@/components/LocalizedLink";
import { AvatarUploadForm } from "@/components/AvatarUploadForm";
import { CheckinLeaderboardDialog } from "@/components/CheckinLeaderboardDialog";
import { DismissibleNotice } from "@/components/DismissibleNotice";
import type { UserWorkspaceKey } from "@/components/UserWorkspace";
import { UserEntitlementList } from "@/components/UserEntitlementList";
import { Pagination } from "@/components/Pagination";
import { listPostgresUserEntitlementsPage } from "@/domains/access/postgres-entitlements";
import { getNoticeDisplaySeconds, getUserAvatarMaxBytes } from "@/lib/config";
import { NO_INDEX_ROBOTS } from "@/lib/seo";
import { getCurrentUser } from "@/lib/user-auth";
import {
  getPostgresDailyCheckinState,
  listPostgresDailyCheckinLeaderboard,
  listPostgresCurrencyTransactionsPage,
} from "@/domains/identity/postgres-user-economy";
...
export default async function AccountPage({ searchParams }: AccountPageProps) {
  ...
  const growth = await getPostgresUserGrowthProgress(database("web"), user.sodaExperience);
  const level = growth.current;
  const checkin = view === "growth" ? await getPostgresDailyCheckinState(database("web"), user.id) : null;
  const transactions = view === "growth"
    ? await listPostgresCurrencyTransactionsPage(database("web"), user.id, Number(params.recordPage || params.sodaPage || 1), 10)
    : null;
  const entitlements = view === "growth"
    ? await listPostgresUserEntitlementsPage(database("web"), user.id, {
        page: Number(params.rightsPage || 1),
        pageSize: 10,
      })
    : null;
  const showCheckinDialog = view === "growth" && params.checkin === "1" && Boolean(checkin?.checkedIn);
  const checkinLeaderboard = showCheckinDialog ? await listPostgresDailyCheckinLeaderboard(database("web")) : null;
  ...
  const permissionLabels = new Map(
    await Promise.all(POSTGRES_USER_PERMISSION_DEFINITIONS.map(async (permission) => [
      permission.key,
      ...
      {view === "profile" ? (
        <article className="userPanel accountPanel accountProfile">
          ...
          <div className="accountFormSections">
            ...
            <section className="accountFormSection accountEmailSection" id="account-email">
              <header>
                <h2>{tr("邮箱")}</h2>
                {emailConfigured && user.email ? (
                  <span className={`accountEmailBadge ${user.emailVerifiedAt ? "isVerified" : "isUnverified"}`}>
                    {user.emailVerifiedAt ? tr("已验证") : tr("未验证")}
                  </span>
                ) : null}
              </header>
              {emailConfigured ? (
                <form className="accountProfileForm accountEmailForm" action={updateAccountEmailAction}>
                  <label>
                    <span>{tr("绑定邮箱")}</span>
                    <input name="email" type="email" defaultValue={user.email || ""} placeholder={tr("请输入邮箱地址")} maxLength={254} />
                  </label>
                  <button className="accountActionButton" type="submit">{user.email ? tr("更新") : tr("绑定")}</button>
                </form>
              ) : (
                <p className="accountEmailUnavailable">{tr("邮件服务暂不可用")}</p>
              )}
            </section>
          ...
          {growthPanel === "rights" && entitlements ? (
            <section className="accountGrowthDetail accountRightsDetail" id="growth-detail">
              <div className="accountPermissionList" aria-label={tr("等级权益")}>
                {POSTGRES_USER_PERMISSION_DEFINITIONS.filter((permission) => level.permissions.includes(permission.key)).map((permission) => (
